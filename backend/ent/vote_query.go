...
func (vq *VoteQuery) QueryOption() *PollOptionQuery {
	query := (&PollOptionClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VoteQuery) Clone() *VoteQuery {
	...
	return &VoteQuery{
		...
		withOption: vq.withOption.Clone(),
...
func (vq *VoteQuery) WithOption(opts ...func(*PollOptionQuery)) *VoteQuery {
	query := (&PollOptionClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	...
	if query := vq.withOption; query != nil {
		if err := vq.loadOption(ctx, query, nodes, nil,
...
func (vq *VoteQuery) loadOption(ctx context.Context, query *PollOptionQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *PollOption)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Vote)
	for i := range nodes {
		fk := nodes[i].OptionID
